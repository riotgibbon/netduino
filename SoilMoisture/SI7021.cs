    ...
    public class SI7021 : ITemperatureSensor, IHumiditySensor
    {
        ...
        public float Humidity
        {
            ...
            private set
            {
                ...
                //
                if ((_updateInterval > 0) && (Math.Abs((int)(_lastNotifiedHumidity - value)) >= HumidityChangeNotificationThreshold))
                {
        ...
        public float Temperature
        {
            ...
            private set
            {
                ...
                //  Check to see if the change merits raising an event.
                //
                if ((_updateInterval > 0) && (Math.Abs((int)(_lastNotifiedTemperature - value)) >= TemperatureChangeNotificationThreshold))
                {
            ...
            float humidityChangeNotificationThreshold = 0.001F, float temperatureChangeNotificationThreshold = 0.001F)
        {
            if (speed > 1000)
            {
                throw new ArgumentOutOfRangeException( "Speed should be between 0 and 1000 KHz");
            }
            if (humidityChangeNotificationThreshold < 0)
            {
                throw new ArgumentOutOfRangeException( "Humidity threshold should be >= 0");
            }
            if (temperatureChangeNotificationThreshold < 0)
            {
                throw new ArgumentOutOfRangeException( "Temperature threshold should be >= 0");
            }
            if ((updateInterval != 0) && (updateInterval < MinimumPollingPeriod))
            {
                throw new ArgumentOutOfRangeException( "Update period should be 0 or >= than " + MinimumPollingPeriod);
            }
