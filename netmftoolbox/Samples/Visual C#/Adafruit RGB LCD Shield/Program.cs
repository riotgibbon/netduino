...
namespace Adafruit_RGB_LCD_Shield
{
    public class Program
    {
        public static void Main()
        {
            ...
            IGPIPort ButtonReset = Mux.Pins[5];
            ...
            while (true)
            {
                if(ButtonLeft.Read()) left++;
                if(ButtonRight.Read()) right++;
                 if(ButtonDown.Read()) down++;
                 if(ButtonUp.Read()) up++;


                Display.ChangePosition(0, 7); Display.Write(left.ToString());
                Display.ChangePosition(1, 7); Display.Write(right.ToString());
                Display.ChangePosition(0, 15); Display.Write(down.ToString());
                Display.ChangePosition(1, 15); Display.Write(up.ToString());
